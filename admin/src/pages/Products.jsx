...
import EditProduct from '../dialogs/EditeProduct';
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {data.map((item) => (
                <tr key={item._id} className="border border-x-0 border-gray-300">
                  <td className="px-6 py-0 text-sm font-medium text-gray-900 line-clamp-1"><Link to={`/product/${item.slug}`}>{item.title}</Link></td>
